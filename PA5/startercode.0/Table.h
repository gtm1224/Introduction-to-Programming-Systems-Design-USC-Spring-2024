...
class Table {
    ...
    //invariants: hashSize > 0;
    // hashSize = hSize when the table is created and stay fixed
    // tableBukets contains NULL pointers when a Table is created
    // numEntry must be equal to the number of present key value pairs
    // hashCode function will always return a number between 0 and hashSize
    // successful insertion will add a new node to the table and increase the numEntry by 1.
    // successful removal will reduce an existing node and reduce the numEntry by 1.
